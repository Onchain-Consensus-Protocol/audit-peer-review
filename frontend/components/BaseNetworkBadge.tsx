...
export function BaseNetworkBadge({ connected }: { connected: boolean }) {
  return (
    <div
      ...
      title={connected ? `Connected to ${auditConfig.chainName}` : `${auditConfig.chainName} not connected`}
      aria-label={connected ? `Connected to ${auditConfig.chainName}` : `${auditConfig.chainName} not connected`}
    >
